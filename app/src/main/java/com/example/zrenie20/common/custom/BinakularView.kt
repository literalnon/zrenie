...
class BinakularView : FrameLayout {
    ...
    override fun drawChild(canvas: Canvas?, child: View?, drawingTime: Long): Boolean {
        ...
        Log.e(
            "BinacularView",
            "drawChild isDraw : ${isDraw}, child : ${child}"
        )

        printHierarchy(child)
        //if (child is ArSceneView) {
            /*mSurfaceView?.holder?.surface?.let { surface ->
                Log.e(
                    "BinacularView",
                    "drawChild surface != null : ${child.width}, ${child.height}"
                )

                child?.renderer?.startMirroring(surface, 0, 0, child.width, child.height)
            }*/
    ...
    fun printHierarchy(
        ...
        Log.e("BinacularView", "printHierarchy : ${child}")

        if (child is ArSceneView && isFirst) {
            Log.e(
                "BinacularView",
                "printHierarchy child is ArSceneView"
            )

            mSurfaceView?.holder?.surface?.let { surface ->
                Log.e(
                    "BinacularView",
                    "printHierarchy surface != null : ${child.width}, ${child.height}"
                )
