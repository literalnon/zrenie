package com.example.zrenie20.location.arcorelocation

import android.app.Activity
import android.os.Handler
import android.util.Log
import com.example.zrenie20.location.arcorelocation.rendering.LocationNode
import com.example.zrenie20.location.arcorelocation.sensor.DeviceLocation
import com.example.zrenie20.location.arcorelocation.sensor.DeviceLocationChanged
import com.example.zrenie20.location.arcorelocation.sensor.DeviceOrientation
import com.example.zrenie20.location.arcorelocation.utils.LocationUtils
...
class LocationScene {
    ...
    private val CALCULATED_TAG = "CALCULATED_LOCATION"
    ...
    constructor(context: Activity, mArSceneView: ArSceneView) {
    ...
    constructor(context: Activity, session: Session?, mArSceneView: ArSceneView) {
        this.context = context
        mSession = session //mArSceneView.getSession();
        this.mArSceneView = mArSceneView
        startCalculationTask()
        deviceLocation = DeviceLocation(context, this)
        deviceOrientation = DeviceOrientation(context)
    ...
    private fun refreshAnchorsIfRequired(frame: Frame) {
        ...
        for (i in mLocationMarkers.indices) {
            try {
                ...
                //[x:-1.314, y:-0.201, z:0.610], q:[x:0.00, y:0.82, z:0.00, w:0.57]
                //pose :t:[x:-0.562, y:-0.161, z:0.207], q:[x:-0.37, y:0.63, z:0.56, w:-0.40]

                Log.e(CALCULATED_TAG, "bearing : ${bearing}")//141.33916
                Log.e(CALCULATED_TAG, "deviceLocation lat, : ${deviceLocation!!.currentBestLocation.latitude}, deviceLocation lon : ${deviceLocation!!.currentBestLocation.longitude}")//deviceLocation lat, : 47.2606736, deviceLocation lon : 39.6869277
                Log.e(CALCULATED_TAG, "marker lat, : ${marker.latitude}, deviceLocation lon : ${marker.longitude}")//marker lat, : 39.694841, deviceLocation lon : 47.258984
                Log.e(CALCULATED_TAG, "bearingAdjustment : ${bearingAdjustment}")
                Log.e(CALCULATED_TAG, "markerDistance : ${markerDistance}")
                ...
                val rotation = markerBearing.toDouble() //Math.floor(markerBearing);
                Log.d(
                ...
                val zRotated = (z * Math.cos(rotationRadian)).toFloat()
                val xRotated = (-(z * Math.sin(rotationRadian))).toFloat()
                ...
                Log.e(
                    "LocationScene", "getDisplayOrientedPose :" + frame.camera
                        .displayOrientedPose
                )
                Log.e(
                    "LocationScene", "getDisplayOrientedPose pose :" + frame.camera.pose
                )
                ...
                Log.e("LocationScene", "getDisplayOrientedPose translation ${frame.camera
                    .displayOrientedPose
                    .compose(translation)
                    .extractTranslation()}")
               ...
               /* val newAnchor = mSession!!.createAnchor(
                   //Pose(floatArrayOf(-1.314f, -0.201f, 0.610f), floatArrayOf(0.00f, 0.82f, 0.00f, 0.57f))
                   Pose(floatArrayOf(1.642f, 1.144f, 1.528f), floatArrayOf(0.00f, 0.37f, 0.00f, 0.93f))
                )*/
